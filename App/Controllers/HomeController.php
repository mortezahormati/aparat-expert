...
//            $sql3 ="select * from comments where video_id=:video_id";
            ...
            if(auth()){
                ...
                $user_favorite_videos = $this->db->query($sql4, [
                   'user_id' => auth()['id']
                ])->fetchAll();
                $user_favorite_video_ids = array_column($user_favorite_videos , 'video_id');
            }
            ...
            loadView('single-page' , [
                ...
                'user_favorite_video_ids' =>$user_favorite_video_ids ?? null
            ]);
        }else{
           redirect('404');
        }
    ...
    protected function videoAddToFavorite($video_id){

        //check not exists
        $sql = "select id from favorites where user_id=:user_id and  video_id=:video_id";
        $favorite_id = $this->db->query($sql,[
            'user_id' => auth()['id'] ,
            'video_id' => $video_id,
        ])->fetch();
        if(empty($favorite_id)){
            $sql3="insert into favorites (user_id,video_id,created_at) values (:user_id, :video_id, :created_at)";
            $this->db->query($sql3,[
                'user_id' => auth()['id'] ,
                'video_id' => $video_id ,
    ...
    public function likes()
    {
        if (strtolower($_SERVER['HTTP_X_REQUESTED_WITH'] ?? '') === 'xmlhttprequest' && $_SERVER['REQUEST_METHOD'] === 'POST'){
            ...
            if($video){
                ...
                //add-to-favorite
                $this->videoAddToFavorite($video['id']);
    ...
    protected function videoRemovedFromFavorites($video_id){
    ...
    public function unlikes()
    {
        ...
        if($video){
            ...
            $this->videoRemovedFromFavorites($video['id']);
